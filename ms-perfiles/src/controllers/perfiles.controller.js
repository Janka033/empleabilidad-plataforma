...
const updateMe = asyncHandler(async (req, res) => {
    ...
    const { nombre, bio, universidad, programa, semestre, habilidades, linkedinUrl, cvUrl } = req.body;
...
const create = asyncHandler(async (req, res) => {
    const { userId, nombre, email, universidad, programa, semestre } = req.body;
...
// GET /perfiles/estudiantes — la coordinadora ve todos los estudiantes (filtro por semestre)
const listarEstudiantes = asyncHandler(async (req, res) => {
    if (req.user.rol !== "admin") return res.status(403).json({ message: "Solo la coordinadora" });
    const { rows } = await pool.query(
        `SELECT id, user_id, nombre, email, universidad, programa, semestre,
                completitud, contratado, empresa_contratante
         FROM perfiles
         WHERE programa IS NOT NULL OR semestre IS NOT NULL
         ORDER BY semestre NULLS LAST, nombre`
    );
    return res.json(rows);
});

// PATCH /perfiles/liberar/:id — la coordinadora académica libera a un estudiante
// (por ejemplo, al finalizar su práctica) para que pueda volver a postularse.
const liberarEstudiante = asyncHandler(async (req, res) => {
    if (req.user.rol !== "admin") {
        return res.status(403).json({ message: "Solo la coordinadora puede liberar estudiantes" });
    }
...
module.exports = { getMe, getById, update, updateMe, create, liberarEstudiante, listarEstudiantes };
